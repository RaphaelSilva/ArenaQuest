...
export function StatCardStreak({ streak }: Props) {
  ...
  return (
    <article
      ...
      aria-label={dict.dashboard.streakCard.label}
    >
      ...
      <div className="mb-3 flex items-center justify-between">
        <span className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: 'var(--aq-text3)' }}>
          {dict.dashboard.streakCard.label}
        </span>
        <span
      ...
      <div className="flex items-end gap-2">
        <span
          ...
          aria-label={`${streak.currentDays} ${streak.currentDays === 1 ? dict.dashboard.streakCard.day : dict.dashboard.streakCard.days} ${dict.dashboard.streakCard.inARow}`}
        >
          {streak.currentDays}
        </span>
        <span className="mb-1 text-sm" style={{ color: 'var(--aq-text2)' }}>
          {streak.currentDays === 1 ? dict.dashboard.streakCard.day : dict.dashboard.streakCard.days} {dict.dashboard.streakCard.inARow}
        </span>
      </div>

      <p className="mt-1 text-[11px]" style={{ color: 'var(--aq-text3)' }}>
        {dict.dashboard.streakCard.personalBest} {streak.longestDays} {streak.longestDays === 1 ? dict.dashboard.streakCard.day : dict.dashboard.streakCard.days}
      </p>

      <div className="mt-3 flex gap-1.5" aria-label={dict.dashboard.streakCard.weekActivity}>
        {pips.map((active, i) => (
