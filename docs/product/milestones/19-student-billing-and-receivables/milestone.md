...
- **Persistence:** D1 + `wrangler d1 migrations`; migration `0026_create_billing_tables.sql` (additive); `D1BillingRepository` implementing `IBillingRepository`; local seed extension under `apps/api/migrations/seed/` (guarded off the deployed path by `apps/api/scripts/check-no-dev-seed.ts`, whose matcher is derived at run time from those seed files — see [backlog/security/03](../../backlog/security/03-dev-seed-guard-hash-drift.task.md)).
